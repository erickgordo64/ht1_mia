...
/*
	esta estructura nos ayudará a llevar el control
	de cuál es el primer espacio libre donde se puede agregar información,
	la información se empieza a agregar despues de esta estructura
	es decir desde el sizeof(control)
*/
type control struct {
	PrimerEspacio int64
}
...
/*
	Tiene un nombre que lo identifica y 5 apuntadores que iran hacia otras
	estructuras
*/
...
func menu() {
	...
	for {
		...
		fmt.Println("4. modificar nodo")
		fmt.Println("5. Leer estructuras.")
		fmt.Println("6. Salir.")
		fmt.Scanf("%d\n", &opcion)
...
func crearEstructura() {
	...
	var posicionEscribir = sizeControl + control.PrimerEspacio*sizeEstructura
	...
	control.PrimerEspacio = control.PrimerEspacio + 1
	file.Seek(0, 0)
...
func buscarEstructura(file *os.File, posicionEstructura int64, origen string) int64 {
	...
	estructuraTemporal := miEstructura{}
...
func modificarNodo() {
	...
	//Toma los datos de entrada del usuario
	reader := bufio.NewReader(os.Stdin)
	fmt.Print("Ingrese el nodo a modificar:")
	origen, _ := reader.ReadString('\n')
	origen = strings.TrimSpace(origen)
